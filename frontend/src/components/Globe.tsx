...
import type { LivePing } from "../hooks/useWarRoomStream";
...
export default function Globe({ state, pings }: Props) {
  ...
  useEffect(() => {
    ...
    g.pointOfView({ lat: 28, lng: -20, altitude: 2.4 }, 0);
  }, []);
  ...
  const points = useMemo(() => {
    const arr =
      state?.arcs.map((a) => ({
        lat: a.startLat,
        lng: a.startLng,
        size: Math.min(0.9, 0.18 + a.conversions / 200),
